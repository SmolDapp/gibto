...
function AvatarBigger({src, address}: {src: string, address: TAddress}): ReactElement {
	const	avatarRef = useRef<HTMLImageElement>(null);
	const	videoExtTypes = ['mp4', 'webm', 'ogg'];
	...
	return (
		<div className={'h-36 w-36 rounded-2xl bg-neutral-200'}>
			{src === '' ? (
				<div
					ref={avatarRef}
					className={'avatarJazz !h-36 !w-36 rounded-2xl object-cover'}>
					<div />
				</div>
			) : videoExtTypes.includes(sanitizedSrc.split('.').pop() || '') ? (
				<video
				...
				<Image
					src={sanitizedSrc}
					alt={''}
					className={'!h-36 !w-36 rounded-2xl object-cover outline outline-neutral-100'}
					width={400}
					height={400}
					unoptimized />
			)}
